...
#    trimmed_reads.close()
    ...
    call("sort %s | uniq > uniq_uniq.txt " %  "uniq_all.txt", shell=True)
...
#    call("rm uniq_1.txt uniq_2.txt uniq_all.txt uniq_trimmed.txt uniq_uniq.txt", shell=True)
    call("rm uniq_1.txt uniq_2.txt uniq_all.txt", shell=True)

    # get reads
    call("seqtk subseq %s %s > %s" % (file1,"uniq_uniq.txt",file1[:-3]+".sel.fq"), shell=True)
    call("seqtk subseq %s %s > %s" % (file2,"uniq_uniq.txt",file2[:-3]+".sel.fq"), shell=True)

    #remove list
#    call("rm %s" % (file1[:-3]+".all.psl.list"), shell=True)
    call("rm uniq_uniq.txt", shell=True)
